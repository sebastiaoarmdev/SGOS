...
type
  ...
  public
    ...
    function FindAll: TObjectList<TOS>;
    function FindById(AId: Integer): TOS;
    procedure Save(AOS: TOS);
    procedure Delete(AOS: TOS);
  end;
...
constructor TOSMapper.Create(AConnection: TFDConnection; ATransaction: TFDTransaction);
begin
  inherited Create;
...
begin
  ...
  try
    try
      ...
      // Recupera o novo ID se for um INSERT
      if AOS.Id = 0 then
      begin
        LQuery.SQL.Text := 'SELECT GEN_ID(GEN_ORDEM_SERVICO_ID, 0) FROM RDB$DATABASE';
        LQuery.Open;
        AOS.Id := LQuery.Fields[0].AsInteger;
        LQuery.Close;
      end;
